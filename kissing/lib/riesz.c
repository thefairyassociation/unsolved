 ...
 * old backtracking gradient descent.
 *
...
#pragma omp critical
    ...
    *mx=m;
    ...
    /* Drop pair contributions below exp(-40); at large s this is almost all pairs. */
...
#pragma omp parallel for reduction(+:E) schedule(static)
    ...
    for(int i=0;i<N;i++){
        ...
        for(int j=i+1;j<N;j++){
            ...
            if(halfs*logr>log_cut) continue;
...
int main(int argc,char**argv){
    ...
    if(argc>5){ FILE*f=fopen(argv[5],"r"); if(!f){perror("seed");return 1;}
        size_t c=0; while(c<Nn && fscanf(f,"%lf",&X[c])==1) c++; fclose(f);
